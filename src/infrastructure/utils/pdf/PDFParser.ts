...
export const validateResumoExtrato = (resumo: ResumoExtrato): ResumoExtrato => {
  return {
    ...
    descontoJudicial: isNaN(resumo.descontoJudicial)
      ? 0
      : resumo.descontoJudicial,
    ...
    descontosEpiCracha: isNaN(resumo.descontosEpiCracha)
      ? 0
      : resumo.descontosEpiCracha,
...
export const validateTrabalho = (trabalho: Trabalho): Trabalho => {
  return {
    ...
    impostoDeRenda: isNaN(trabalho.impostoDeRenda)
      ? 0
      : trabalho.impostoDeRenda,
    descontoJudicial: isNaN(trabalho.descontoJudicial)
      ? 0
      : trabalho.descontoJudicial,
    das: isNaN(trabalho.das) ? 0 : trabalho.das,
    mensal: isNaN(trabalho.mensal) ? 0 : trabalho.mensal,
    impostoSindical: isNaN(trabalho.impostoSindical)
      ? 0
      : trabalho.impostoSindical,
    descontosEpiCracha: isNaN(trabalho.descontosEpiCracha)
      ? 0
      : trabalho.descontosEpiCracha,
    liquido: isNaN(trabalho.liquido) ? 0 : trabalho.liquido,
    ferias: isNaN(trabalho.ferias) ? 0 : trabalho.ferias,
    decimoTerceiro: isNaN(trabalho.decimoTerceiro)
      ? 0
      : trabalho.decimoTerceiro,
    encargosDecimo: isNaN(trabalho.encargosDecimo)
      ? 0
      : trabalho.encargosDecimo,
...
export const extractHeader = (
  ...
  for (let i = 0; i < textContent.length; i++) {
    if (textContent[i].trim() === 'EXTRATO ANALÍTICO') {
      ...
      if (i + 1 < textContent.length) {
        const matriculaNomeMatch =
          textContent[i + 1].match(/^(\d{3}-\d+)\s+(.+)$/)
        if (matriculaNomeMatch) {
          matricula = matriculaNomeMatch[1]
          nome = matriculaNomeMatch[2]
          console.log(
            `Encontrado padrão padrão: matricula=${matricula}, nome=${nome}`,
          )
        }
      }
      ...
      if (i + 2 < textContent.length) {
        const mesAnoMatch = textContent[i + 2].match(
  ...
  for (const line of textContent) {
    if (!matricula) {
      ...
      if (match) {
        ...
        const afterMatricula = line
          .substring(line.indexOf(match[1]) + match[1].length)
          .trim()
        ...
        console.log(
          `Encontrado individual: matricula=${matricula}, possível nome=${nome}`,
        )
      }
    }
    ...
    if (!categoria) {
      ...
      if (categoria)
        console.log(`Encontrado individual: categoria=${categoria}`)
    }
  ...
  console.log(
    `Resumo do cabeçalho encontrado: Matrícula=${matricula}, Nome=${nome}, Mês=${mes}, Ano=${ano}, Categoria=${categoria}`,
  )
  ...
  if (!matricula || !nome || !mes || !ano || !categoria) {
    ...
    console.log(
      'Tentando abordagem alternativa para encontrar informações de cabeçalho...',
    )
    ...
    if (!nome && matricula) {
      ...
      const afterMatricula = combinedText
        .substring(combinedText.indexOf(matricula) + matricula.length)
        .trim()
    ...
    if (!mes || !ano) {
      ...
      for (const mesAbrev of mesesAbrev) {
        if (combinedText.includes(mesAbrev)) {
          ...
          const textAroundMonth = combinedText.substring(
            Math.max(0, mesIndex - 5),
            Math.min(combinedText.length, mesIndex + 10),
          )
    ...
    console.log(
      `Resumo final do cabeçalho: Matrícula=${matricula}, Nome=${nome}, Mês=${mes}, Ano=${ano}, Categoria=${categoria}`,
    )
  }
  ...
  if (!matricula || !nome || !mes || !ano || !categoria) {
    console.log(
      'AVISO: Usando valores padrão para campos de cabeçalho faltantes',
    )
...
const extractStructuredData = (pdfData: any): StructuredData => {
  ...
  const allTextLines: string[] = allLines
    .map((line) =>
      line
        .map((item) => item.text)
        .join(' ')
  ...
  if (summaryStartIndex === -1) {
    for (let i = allTextLines.length - 10; i < allTextLines.length; i++) {
      ...
      if (
        !line.includes('OGMO') &&
        !line.includes('EMAIL') &&
        !line.includes('PORTUÁRIO') &&
        !line.includes('SETOR DE')
      ) {
        ...
        if (
          numericParts.length >= 10 &&
          numericParts.length / parts.length >= 0.8
        ) {
          summaryStartIndex = i
          break
        }
  ...
  console.log(
    `Seções identificadas: header=${headerLines.length}, data=${dataLines.length}, summary=${summaryLines.length}`,
  )
...
const extractShipName = (record: StructuredRecord): string => {
  ...
  if (record.lines.length > 1) {
    ...
    for (let i = 1; i < record.lines.length - 1; i++) {
      ...
      const nameElements = line.filter(
        (el) =>
          el.text.length > 2 &&
          /^[A-Z]/.test(el.text) &&
          !/^\d+/.test(el.text) &&
...
const processStructuredRecord = (record: StructuredRecord): Trabalho | null => {
  ...
  if (parts.length < 15) {
    console.log(
      `Registro muito curto para ser um trabalho válido para dia ${dia}, folha ${folha}`,
    )
    return null
  }
  ...
  for (let i = 0; i < parts.length - 3; i++) {
    if (
      FUNCOES_VALIDAS.includes(parts[i]) &&
      i + 1 < parts.length &&
      TURNOS_VALIDOS.includes(parts[i + 1]) &&
      i + 2 < parts.length &&
      /^[1-3]$/.test(parts[i + 2]) &&
      i + 3 < parts.length &&
      /^\d{2}\/\d{2}$/.test(parts[i + 3])
    ) {
      funIndex = i
      break
    }
    ...
    for (let i = parts.length - 1; i > 3; i--) {
      if (/^\d{2}\/\d{2}$/.test(parts[i])) {
        ...
        if (
          ...
          /^[1-3]$/.test(parts[i - 1]) &&
          i - 2 >= 0 &&
          TURNOS_VALIDOS.includes(parts[i - 2]) &&
          i - 3 >= 0 &&
          FUNCOES_VALIDAS.includes(parts[i - 3])
        ) {
          funIndex = i - 3
          break
        }
  ...
  if (numericValues.length < 13) {
    ...
    if (lastThirteen.length === 13) {
      console.log(
        `Usando os últimos 13 valores numéricos para dia ${dia}, folha ${folha}`,
      )
      for (let i = 0; i < 13; i++) {
  ...
  // Log para debug
  console.log(
    `Extração de trabalho: dia=${dia}, folha=${folha}, tomador=${tomador}, pasta="${pasta}", fun=${fun}, tur=${tur}, ter=${ter}, pagto=${pagto}`,
  )
...
const extractSummary = (
  summaryLines: string[],
): { folhasComplementos: ResumoExtrato; revisadas: ResumoExtrato } | null => {
  ...
  for (const line of summaryLines) {
    ...
    if (
      trimmedLine.includes('OGMO') ||
      trimmedLine.includes('EMAIL') ||
      trimmedLine.includes('PORTUÁRIO') ||
      trimmedLine.includes('SETOR DE') ||
      /^\d{2}\/\d{2}\/\d{4}/.test(trimmedLine) ||
    ...
    if (trimmedLine.includes('Folhas/Complementos')) {
      ...
      console.log(
        `Encontrada linha explícita Folhas/Complementos: "${trimmedLine}"`,
      )
    }
    ...
    if (
      !totalLine &&
      !trimmedLine.includes('Folhas/Complementos') &&
      !trimmedLine.includes('Revisadas')
    ) {
      ...
      if (
        numericParts.length >= 10 &&
        numericParts.length / parts.length >= 0.9
      ) {
        totalLine = trimmedLine
        console.log(
          `Encontrada possível linha de total (apenas valores): "${trimmedLine}"`,
        )
      }
    }
  }
  ...
  if (!folhasComplementosLine || !revisadasLine) {
    ...
    console.log(
      `Encontradas ${numericLines.length} linhas com muitos valores numéricos`,
    )
    ...
    if (numericLines.length >= 1) {
      ...
      if (!totalLine) {
        ...
        console.log(
          `Definindo primeira linha numérica como total: "${totalLine}"`,
        )
      }
      ...
      if (!folhasComplementosLine && numericLines.length >= 2) {
        ...
        console.log(
          `Definindo segunda linha numérica como Folhas/Complementos: "${folhasComplementosLine}"`,
        )
      }
      // Se não temos mais de uma linha numérica, usar a linha de total como Folhas/Complementos
        ...
        console.log(
          `Usando linha de total como Folhas/Complementos: "${folhasComplementosLine}"`,
        )
      }
      ...
      if (!revisadasLine && numericLines.length >= 3) {
        ...
        console.log(
          `Definindo terceira linha numérica como Revisadas: "${revisadasLine}"`,
        )
      }
  ...
  if (!folhasComplementosLine) {
    console.log(
      'Não foi possível encontrar a linha de Folhas/Complementos. Usando valores calculados.',
    )
    return null
  }
  ...
  const extractValues = (line: string): number[] => {
    ...
    const numericPart = line
      .replace(/Folhas\/Complementos|Revisadas/g, '')
      .trim()
  ...
  const revisadasValues = revisadasLine
    ? extractValues(revisadasLine)
    : Array(13).fill(0)

  console.log(
    `Valores extraídos - Folhas/Complementos: ${folhasValues.length} valores, Revisadas: ${revisadasValues.length} valores`,
  )
...
export const parseExtratoAnalitico = (filePath: string): Promise<Extrato> => {
  return new Promise((resolve, reject) => {
    ...
    pdfParser.on('pdfParser_dataReady', (pdfData: any) => {
      try {
        console.log(
          `PDF carregado, iniciando extração usando abordagem estruturada baseada em seções...`,
        )
        ...
        const { matricula, nome, mes, ano, categoria } = extractHeader(
          structuredData.headers,
        )
        console.log(
          `Dados de cabeçalho: ${matricula}, ${nome}, ${mes}/${ano}, ${categoria}`,
        )
        ...
        for (const record of structuredData.records) {
          try {
            ...
            if (trabalho) {
              ...
              const isDuplicate = trabalhos.some(
                (t) => t.dia === trabalho.dia && t.folha === trabalho.folha,
              )

              if (!isDuplicate) {
                trabalhos.push(trabalho)
                console.log(
                  `Trabalho adicionado: Dia ${trabalho.dia}, Folha ${trabalho.folha}, Tomador ${trabalho.tomador}, Pasta "${trabalho.pasta}"`,
                )
              } else {
                console.log(
                  `Trabalho duplicado ignorado: Dia ${trabalho.dia}, Folha ${trabalho.folha}`,
                )
              }
            }
          } catch (err) {
        ...
        const summaryData = structuredData.summaryLines
          ? extractSummary(structuredData.summaryLines)
          : null
        ...
        if (!summaryData) {
          ...
          folhasComplementos = {
            baseDeCalculo: trabalhos.reduce(
              (sum, t) => sum + (isNaN(t.baseDeCalculo) ? 0 : t.baseDeCalculo),
              0,
            ),
            inss: trabalhos.reduce(
              (sum, t) => sum + (isNaN(t.inss) ? 0 : t.inss),
              0,
            ),
            impostoDeRenda: trabalhos.reduce(
              (sum, t) =>
                sum + (isNaN(t.impostoDeRenda) ? 0 : t.impostoDeRenda),
              0,
            ),
            descontoJudicial: trabalhos.reduce(
              (sum, t) =>
                sum + (isNaN(t.descontoJudicial) ? 0 : t.descontoJudicial),
              0,
            ),
            das: trabalhos.reduce(
              (sum, t) => sum + (isNaN(t.das) ? 0 : t.das),
              0,
            ),
            mensal: trabalhos.reduce(
              (sum, t) => sum + (isNaN(t.mensal) ? 0 : t.mensal),
              0,
            ),
            impostoSindical: trabalhos.reduce(
              (sum, t) =>
                sum + (isNaN(t.impostoSindical) ? 0 : t.impostoSindical),
              0,
            ),
            descontosEpiCracha: trabalhos.reduce(
              (sum, t) =>
                sum + (isNaN(t.descontosEpiCracha) ? 0 : t.descontosEpiCracha),
              0,
            ),
            liquido: trabalhos.reduce(
              (sum, t) => sum + (isNaN(t.liquido) ? 0 : t.liquido),
              0,
            ),
            ferias: trabalhos.reduce(
              (sum, t) => sum + (isNaN(t.ferias) ? 0 : t.ferias),
              0,
            ),
            decimoTerceiro: trabalhos.reduce(
              (sum, t) =>
                sum + (isNaN(t.decimoTerceiro) ? 0 : t.decimoTerceiro),
              0,
            ),
            encargosDecimo: trabalhos.reduce(
              (sum, t) =>
                sum + (isNaN(t.encargosDecimo) ? 0 : t.encargosDecimo),
              0,
            ),
            fgts: trabalhos.reduce(
              (sum, t) => sum + (isNaN(t.fgts) ? 0 : t.fgts),
              0,
            ),
          }
        ...
        console.log(
          `Extrato processado com sucesso. Total de trabalhos: ${trabalhos.length}`,
        )
        console.log(
          `Resumo - Base de Cálculo: ${folhasComplementos.baseDeCalculo}, Líquido: ${folhasComplementos.liquido}`,
        )

        // Comparar totais calculados com os extraídos para validação
        if (trabalhos.length > 0) {
          const calculatedTotal = trabalhos.reduce(
            (sum, t) => sum + (isNaN(t.baseDeCalculo) ? 0 : t.baseDeCalculo),
            0,
          )
          ...
          console.log(
            `Validação: Total calculado=${calculatedTotal}, Total extraído=${extractedTotal}`,
          )
          ...
          const percentDifference =
            extractedTotal > 0 ? (difference / extractedTotal) * 100 : 0

          if (percentDifference > 10 && extractedTotal > 0) {
            console.log(
              `Aviso: Diferença significativa (${percentDifference.toFixed(2)}%) entre total calculado e extraído`,
            )
          }
        }
        ...
        if (error instanceof Error) {
          reject(
            new PDFParserError(
              `Erro ao processar dados do PDF: ${error.message}`,
            ),
          )
        } else {
          reject(
            new PDFParserError('Erro desconhecido ao processar dados do PDF'),
          )
        }
      }
    })
