...
export class AnalyticsService {
  ...
  static async getTomadoresAnalytics(extratoIds: string[]) {
    try {
      ...
      return Object.values(tomadoresMap).sort((a, b) => b.valorTotal - a.valorTotal);
    } catch (error) {
  ...
  static async getSalaryBreakdown(startDate: Date, endDate: Date) {
    try {
      // Converter datas para strings no formato YYYY-MM-DD para comparação no MongoDB
      ...
      const aggregation = await ExtratoModel.aggregate([
        {
          $match: {
            $or: [
              { 
                ano: { $gte: startDate.getFullYear().toString(), $lte: endDate.getFullYear().toString() },
                mes: { 
      ...
      // Converter para o formato esperado pela vizualização
      return [
        ...
        { name: 'Outros', value: (result.descontosEpiCracha || 0) + (result.mensal || 0) }
  ...
  static async getWeeklyWorkDistribution(startDate: Date, endDate: Date) {
    try {
      ...
      // Converter para o formato esperado pela visualização
      return Object.entries(weeklyData).map(([week, months]) => {
        return {
          week: `Semana ${week}`,
          ...months
        };
  ...
  static async getDashboardSummary(startDate: Date, endDate: Date) {
    try {
      ...
      // Calcular médias
      const mediaBrutoFaina = totalTrabalhos > 0 ? totalBruto / totalTrabalhos : 0;
      const mediaLiquidoFaina = totalTrabalhos > 0 ? totalLiquido / totalTrabalhos : 0;
   ...
   */
  private static getMonthsBetween(startMonth: number, startYear: number, endMonth: number, endYear: number): string[] {
